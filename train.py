...
if __name__ == '__main__':
	...
	for source in source_domain:
		...
		source_clf[source]['optim'] = optim.Adam(list(source_clf[source]['c1'].parameters()) + list(source_clf[source]['c2'].parameters()), lr=1e-4, weight_decay=0.0005)
	...
	for ep in range(EP):
		...
		for batch_index, (src_batch, tar_batch) in enumerate(zip(zip(*source_dataloader_list), target_dataloader)):
			...
			for index, source in enumerate(source_domain):
				...
				source_clf[source_domain[index]]['optim'].step()
			...
			for k in range(1, 3):
				for i_index, batch in enumerate(src_batch):
					...
					e_src = torch.mean(src_feature**k)
					e_tar = torch.mean(tar_feature**k)
					...
					for j_index, other_batch in enumerate(src_batch[i_index:]):
						...
						e_other = torch.mean(other_feature**k)
						m2_dist = e_src.dist(e_other)
						m2_loss += m2_dist

			loss_m = m1_loss/N + m2_loss/N/(N-1)*2
			 
			loss = loss_cls + 0.5 * loss_m
			if batch_index % 5 == 0:
				print('loss_m:' , loss_m)
			...
			for index, batch in enumerate(src_batch):
				...
				pred = source_clf[source_domain[index]]['c2'](feature)

				clf_loss = loss_extractor(pred, y)
				
				pred_c1 = source_clf[source_domain[index]]['c1'](tar_feature)
				pred_c2 = source_clf[source_domain[index]]['c2'](tar_feature)
				
				discrepency_loss = torch.mean(abs(F.softmax(pred_c1, dim=1) - F.softmax(pred_c2, dim=1)))
